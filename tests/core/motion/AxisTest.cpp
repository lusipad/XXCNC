...
protected:
    void SetUp() override {
        ...
        axis_ = std::make_unique<Axis>("X", params);
    }

    std::unique_ptr<Axis> axis_;
};

TEST_F(AxisTest, InitialState) {
...
TEST_F(AxisTest, AccelerationControl) {
    axis_->enable();
...
TEST_F(AxisTest, SoftLimitProtection) {
    axis_->enable();
...
TEST_F(AxisTest, PositionControl) {
    EXPECT_TRUE(axis_->enable());
